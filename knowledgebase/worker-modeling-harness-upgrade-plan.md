...
2026-09-24：R0–R5 修复已落地，R6 按已通过类别 1 次、未通过类别 3 次执行。
实际结果见 [本轮执行记录](worker-modeling-r0-r6-execution.md)，不能将代码部署等同于 P4 验收通过。

日期：2026-09-22。执行环境：Windows worker，Blender 5.2.1 LTS，目标引擎 UE 5.8。
状态：**P0–P3 已实现；空闲 worker 已部署 cf86fe4 并启用 V2。2026-09-24 完成六类各三次重复样本和失败分类，14/18 最终通过；P4 发布验收仍有缺口。仓库默认开关仍为关闭。**
...
最新部署与重复样本证据见 [P4 校准记录](worker-modeling-p4-calibration.md)。
当前校准已发现碰撞通行覆盖缺口、骨骼导入辅助物误拒绝、固定图评审不一致，以及响应格式错误触发重建。
相同完整规格与预算的旧流程对照仍待补齐，发布验收尚未满足。
针对上述证据的分批修复方案见 [P4 具体修复计划](worker-modeling-p4-repair-plan.md)，当前尚未实施。
...
仓库默认开关仍为 `0`，本机运行时已按操作授权启用为 `1`。首轮每类三次重复样本已完成；同预算对照和已发现检查器/评审问题尚未解决，不将新流程宣称为普遍稳定。显式 v2 合同可以使用已实现的流程；未支持的必需检查返回 GAP。
