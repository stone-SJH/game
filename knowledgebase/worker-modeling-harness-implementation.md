...
2026-09-24：R0–R5 修复已提交，R6 正在执行；固定图校准仍存在服务可用性阻塞。
当前部署、验证证据及未完成门槛见 [本轮执行记录](worker-modeling-r0-r6-execution.md)。
...
2026-09-23 已在空闲 worker 部署 `cf86fe49a751bfc7dd8ff01ccc88c05411f9c241`，
本机运行时已启用 `MODELING_HARNESS_V2_ENABLED=1`。2026-09-24 已完成六类各三次重复样本，
14/18 最终通过，接受产物 14/14 重验通过；改色任务 3/3 通过，其中复用路线仅 2/3。
失败分类及完整证据见
[P4 校准记录](worker-modeling-p4-calibration.md)；下方首次试跑记录不代表本轮最终结果。
P4 已复现门洞碰撞覆盖缺口、glTF 骨骼显示辅助物误拒绝、固定图片的视觉评审不一致，
以及响应格式错误消耗作者预算并触发重建的问题。
代码已实现不等于这些能力已完成稳定性校准；本轮原始失败与反例均保留。
后续实施依据见 [P4 具体修复计划](worker-modeling-p4-repair-plan.md)，按 R0–R6 分批修复和验收。
...
`MODELING_HARNESS_V2_ENABLED` 的仓库默认值为 `0`；本机部署的运行时配置已启用为 `1`。
显式提供 v2 合同的隔离任务也可以使用新实现。
