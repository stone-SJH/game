...
更新：2026-09-25。R0–R5 代码完成，R6 三组共 36 个固定建模槽位全部结束；P4 全范围验收未通过。
完整结果、共同检查纠错和部署边界见 [R6 结果报告](worker-modeling-r6-results-20260925.md)。下文保留按时间顺序的原始执行记录。
...
18:06 CST，对照完成 8/24 槽，参考低模基线正在运行。复用槽在线结果已经齐备：
基线 2/3 PASS（160,673 ms 失败；1,651,379 / 940,123 ms 通过），
完整合同单阶段 3/3 PASS（609,838 / 693,924 / 476,261 ms），成功槽均选择 reuse_blender。
三组的独立几何/材质检查尚未执行，因此暂不将路线标签等同于保形修改验收。
...
19:30 CST，对照完成 11/24 槽。参考低模基线、完整合同单阶段均在线通过，终态报告耗时分别
2,224,978 / 593,900 ms；基线前两次 final 调用受 503 影响，第三次完成。原始参考图片和四条要求未改，
没有预登记数值 silhouette mask，因此此处参考要求的在线结论来自逐条视觉评审，不能称为 IoU 校准通过。
基线模块也在线 DCC 通过，耗时 1,602,319 ms；这不是 UE 验收结论。单阶段模块第一作者调用因 503 失败，
第二次尝试进行中。快照 `r6-baseline-audit-03.json`、`r6-single-stage-audit-03.json` 输入/规格/已接受文件完整性通过。

最终审计预检发现单阶段清单的 variant 为 `single-stage-full-contract`，不同于目录和任务 id 前缀
`single-stage`。仅停止尚未执行审计的等待进程，修正映射并校验两组各 12 槽后重新排队；尚无最终报告，
建模队列、复验登记及模型结果均未修改。错误预检在读取登记时退出，没有调用模型或生成审计报告。

20:04 CST，对照完成 13/24 槽，在线 10 PASS / 3 FAIL；单阶段有机第 1 槽运行中。
单阶段模块第二次作者尝试通过，耗时 1,268,908 ms，21 份接受文件；基线模块 33 份接受文件。
两者都是 DCC 在线结果，不等于 UE 或新增通行验收通过。
基线有机第 1 槽首次作者尝试通过，耗时 766,458 ms。维护者补看实际 GLB 的顶视与透视截图，
可见同心年轮、三条展开根部和连续衔接；这是补充观察，不能替代共同检查器或人工标注真值。
共同复验的 27 份检查器、6 份运行时文件、模型覆盖项及 72 个允许追加 trust 的项目路径已重新核对通过。

20:20 CST，对照完成 14/24 槽，在线 11 PASS / 3 FAIL；基线有机第 2 槽运行中。
单阶段有机第 1 槽第二次作者尝试通过，耗时 1,512,140 ms，19 份接受文件。
...
21:16 CST，对照完成 18/24 槽，在线 15 PASS / 3 FAIL，开始基线绑定第 1 槽。
基线有机三槽均在线通过，耗时 766,458 / 1,559,231 / 575,093 ms；
完整合同单阶段有机三槽均在线通过，耗时 1,512,140 / 666,291 / 606,081 ms。
单阶段第 1 槽有一次有效 UV GAP 后修复，其余两槽首次作者尝试通过；共同终态复验尚未执行。
候选有机 0/3 的原失败继续保留，候选与对照执行时段的服务可用性不同，不能据此给出流程因果优劣结论。
