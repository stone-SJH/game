...
test('worker binding and allocation prevent duplicate dispatch; cancellation fences late success', async () => {
  const task = await create();
  const queued = await request('/v1/worker/status', { headers: agentHeaders });
  assert.equal(queued.status, 200);
  assert.equal(queued.value.active, null);
  assert.equal(queued.value.queuedJobs, 1);
  await db.query('INSERT INTO workers(worker_id,token_hash) VALUES($1,$2)', ['worker-other-queue', digest('other-queue-secret')]);
  await db.query('UPDATE workspaces SET worker_id=$2 WHERE workspace_id=$1', [task.workspaceId, 'worker-other-queue']);
  assert.equal((await request('/v1/worker/status', { headers: agentHeaders })).value.queuedJobs, 0);
  await db.query('UPDATE workspaces SET worker_id=NULL WHERE workspace_id=$1', [task.workspaceId]);
  const job = await claim();
  assert.equal(job.taskId, task.taskId);
  const beforeStatus = (await db.query('SELECT lease_until FROM jobs WHERE job_id=$1', [job.jobId])).rows[0];
  const seenBefore = (await db.query('SELECT last_seen_at FROM workers WHERE worker_id=$1', [workerId])).rows[0];
  const running = (await request('/v1/worker/status', { headers: agentHeaders })).value;
  assert.equal(running.active.taskId, task.taskId);
  assert.equal(running.active.taskStatus, 'RUNNING');
  assert.equal(running.active.workspaceId, task.workspaceId);
  assert.equal(running.queuedJobs, 0);
  assert.equal('leaseToken' in running.active, false);
  assert.deepEqual((await db.query('SELECT lease_until FROM jobs WHERE job_id=$1', [job.jobId])).rows[0], beforeStatus);
  assert.deepEqual((await db.query('SELECT last_seen_at FROM workers WHERE worker_id=$1', [workerId])).rows[0], seenBefore);
  ...
  assert.equal((await request('/v1/worker/status', { headers: agentHeaders })).value.active.taskStatus, 'CANCELING');
  ...
  assert.equal((await request('/v1/worker/status', { headers: agentHeaders })).value.active, null);
  assert.equal((await request('/v1/worker/step-result', { headers: agentHeaders, data: finish })).value.status, 'CANCELED');
...
test('task artifacts expose task revisions while run filtering remains available', async () => {
  ...
  assert.deepEqual(current.artifacts.map(item => item.taskRevision), [2]);
  assert.ok(current.artifacts.every(item => item.createdAt));
  assert.deepEqual(current.artifactFacets.map(item => [item.taskRevision, item.count]), [[2, 1], [1, 3]]);
  ...
  const firstRevision = (await request(`/v1/tasks/${task.taskId}/artifacts?limit=100&revision=1`, { account: alice })).value;
  assert.deepEqual(firstRevision.artifacts.map(item => item.name).sort(), ['first-run.txt', 'iteration-monitor-2.json', 'playable-package-iteration-002.zip']);
  assert.equal(firstRevision.count, 3);
  const firstRevisionPage = (await request(`/v1/tasks/${task.taskId}/artifacts?limit=1&revision=1`, { account: alice })).value;
  assert.equal(firstRevisionPage.artifacts.length, 1);
  assert.equal(firstRevisionPage.count, 3);
  const packages = (await request(`/v1/tasks/${task.taskId}/artifacts?limit=100&revision=1&type=playable-package`, { account: alice })).value;
  assert.deepEqual(packages.artifacts.map(item => item.name), ['playable-package-iteration-002.zip']);
