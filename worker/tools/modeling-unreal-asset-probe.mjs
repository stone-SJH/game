// Import one real accepted static benchmark asset into an isolated UE project and run the host gate.
import fs from 'node:fs/promises';
import path from 'node:path';
import { runCommand } from '../agent/process-runner.mjs';
import { atomicJson, readJson, localPath, hashFile, repositoryRoot, agentEnvironment } from '../agent/modeling-io.mjs';
...
if(process.argv.length!==4)throw new Error('Usage: node modeling-unreal-asset-probe.mjs <accepted-project> <new-output>');
...
const sourceAsset=summary?.assets?.find(a=>a.contract?.runtime.engine==='unreal'&&a.contract.runtime.profile==='fbx-static');
if(!sourceAsset)throw new Error('An accepted static FBX asset targeting Unreal is required.');
const asset=structuredClone(sourceAsset),output=path.join(project,'run');await fs.mkdir(output);
const modelDir=path.dirname(asset.files.find(f=>f.path.endsWith('/model.fbx')).path);
...
async function step(name,exe,args,timeoutMs,cwd=project,accepts,options={}){
 ...
 if(!r.stopConfirmed||r.exitCode!==0||r.error||r.timedOut)throw new Error(`${name}: ${r.error||r.stderr.slice(-1500)||r.stdout.slice(-1500)}`);return r;
}
...
const result=await validateUnrealModels({summary:{assets:[asset]},project,output,unreal,projectFile,step,signal:controller.signal,invocation:codexInvocation([]),attempt:1});
