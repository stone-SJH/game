...
The browser app is deployed as the sibling `app/` directory from the same Git release as the
controller. Use `controller/deploy/deploy-controller.sh`; do not copy a separate app directory or
release tar. Nginx may serve the static files or proxy them to the controller. There is no frontend
build step or embedded secret.
