...
function signedOut() {
  ...
  $('task-list').replaceChildren(); $('detail').replaceChildren(); $('create-dialog').close();
}
...
async function refreshDetail() {
  ...
  for (const [label, value] of [['Worker',task.workerId || 'Waiting for assigned worker'], ['Created',date(task.createdAt)], ['Workspace',task.workspaceId || 'Unassigned'], ['Task',task.taskId]]) {
    const group = node('div'); group.append(node('dt',label),node('dd',value)); details.append(group);
  }
  pane.append(details,node('h3','Activity'));
